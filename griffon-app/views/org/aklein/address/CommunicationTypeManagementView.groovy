...
panel(id: 'content') {
    ...
    widget(id: 'mainPanel', metaComponentFix('masterlist',
            ...
            columnModel: columnModelFix {
                column("name", modelIndex: 0, headerValue: app.getMessage('communicationTypeManagement.table.header.name.text', 'Name'), width: [100])
            },
            list: model.list,
            filter: [name: { CommunicationType item, String value, String filter -> !(filter && !value?.toLowerCase()?.contains(filter.toLowerCase())) }],
            comparator: { a, b -> a.name <=> b.name },
            messagePrefix: 'communicationTypeManagement.',
